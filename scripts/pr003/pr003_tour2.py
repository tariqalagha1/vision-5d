"""PR003 v2 — corrected shot plan.

Root cause (v1): cameras too high / looking too steeply down at low furniture,
so the sofa+table fell out of frame. Fix: eye-level cameras, near-horizontal
views, targets anchored on the furniture group.

Scene (Blender): room x[0,5] y[-4,0] z[0,2.7], OPEN back at y=-4.
Sofa x[1.5,3.5] y[-3.7,-2.8] z[0.1,1.2] (faces open back).
Table x[1.9,3.1] y[-2.4,-1.6] z[0.1,0.75].
Mode: 'diag' = 1 midpoint frame/shot; 'full' = all frames.
"""
...
# Furniture group center for look targets
...
try:
    ...
    for s in SHOTS:
        ...
        if MODE == "diag":
            ...
            fp = os.path.join(shot_dir, f"mid2_{s['id']}.png")
...
with open(os.path.join(OUT_DIR, "tour2_result.json"), "w") as f:
