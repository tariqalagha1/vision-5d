...
SX = 304.8/29.08; SY = 304.8/24.38

bpy.ops.object.select_all(action='SELECT'); bpy.ops.object.delete()
bpy.ops.import_scene.gltf(filepath=GLB)
meshes=[o for o in bpy.context.scene.objects if o.type=='MESH']

# keep only wall_solid (the correctly-placed building structure); hide the rest
# door_element/window_element are misplaced near origin; floor/ceiling/roof collapsed to a point
keep=[]; hide=[]
for o in meshes:
    pfx=o.name.split('.')[0]
    if pfx=='wall_solid':
...
print(f"KEEP={len(keep)} HIDE={len(hide)}")
print(f"extent mm={[round(v) for v in ext]} = X {W/1000:.1f}m Y {L/1000:.1f}m H {H/1000:.1f}m")
...
bpy.context.view_layer.update()
sc.render.filepath=OUT+"/overview_v3.png"
bpy.ops.render.render(write_still=True)
print("DONE overview_v3.png")
