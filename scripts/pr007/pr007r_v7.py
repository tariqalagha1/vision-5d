"""PR007R v7 — motion: dolly-in using Track To constraint (guaranteed movement)."""
...
# empty target in the living furniture cluster
bpy.ops.object.empty_add(location=(4.2, 36.2, 1.0))
tgt = bpy.context.object

# camera with Track To constraint (guaranteed look-at)
bpy.ops.object.camera_add(location=(2.5, 30.5, 3.6))
...
N = 72  # 3s
start = Vector((2.5, 30.5, 3.6))
end = Vector((3.9, 34.6, 2.8))
for f in range(1, N + 1):
    t = (f - 1) / (N - 1)
    cam.location = start.lerp(end, t)
