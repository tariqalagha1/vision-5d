#!/usr/bin/env python3
import json, time, urllib.request, http.cookiejar, struct, os, glob
BASE="http://localhost:8000"; PID="1164369e-20ae-4557-bfe9-991e8b91f910"
GID="374222af-d5ad-43ca-a8af-67c35056f940"
...
r=post(f"/api/v3/projects/{PID}/geometry/reconstruct?source_graph_id={GID}"); gj=r.get("job_id"); print("  job", gj)
g=poll(gj); print("  state", g.get("state"))
print("scene3d reconstruct ...")
r=post(f"/api/v4/projects/{PID}/scene/reconstruct"); sj=r.get("job_id"); print("  job", sj)
s=poll(sj); print("  state", s.get("state"))
