...
(async () => {
  ...
  ws.addEventListener('message',ev=>{const m=JSON.parse(typeof ev.data==='string'?ev.data:Buffer.from(ev.data).toString()); if(m.id&&pending.has(m.id)){pending.get(m.id)(m);pending.delete(m.id);} if(m.method==='Network.requestWillBeSent'){reqs.push(m.params.request.url.split('/').pop()+' '+m.params.request.url.split('/').slice(-2)[0]);} if(m.method==='Network.responseReceived'){const u=m.params.response.url; if(u.includes('.glb')||u.includes('.js')) reqs.push('RESP '+(m.params.response.status)+' '+u.split('/').pop());} });
  ...
  console.log('NETWORK (last 30):', JSON.stringify(reqs.slice(-30), null, 1));
